...
class IAalgo2 implements IAInterface{
    ...
    public Grille resoudre(Grille grille) {
        ...
        for (int y = 1; y < height-1; y++)
            for (int x = 1; x < width-1; x++)
                if (laGrille.isJouable(x, y) && (!(laGrille.isJouable(x, y-1) || laGrille.isJouable(x, y+1))) && (!(laGrille.isJouable( x-1, y) || laGrille.isJouable(x+1, y))))
